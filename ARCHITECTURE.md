...
The same state-versus-methodology separation applies one level down, inside the
private instance itself: `skills/` holds methodology only, and anything that
changes as the search runs — the session log, an open-threads file read at every
session start, dated snapshots, generated views of the tracker — lives in a
separate `data/` directory. The tracker CSV is the single hand-edited
application record; human-readable views of it are generated by script rather
than maintained as parallel files, which removes an entire class of mirror
drift (two files describing the same applications, updated separately,
disagreeing eventually — a failure this project hit once in practice before
adopting the rule).
...
- Three rituals are foreseen and deliberately not yet built, because the
  search hasn't reached them and speculative runbooks encode guesses
  instead of experience: the interview loop (per-round prep, follow-ups,
  debrief capture), offer evaluation and negotiation, and the search
  wind-down (withdrawing active applications, notifying warm contacts,
  archiving the instance). Each gets built when its stage arrives, from
  what actually happens there.
- The resume builder is now generalized: `framework/scripts/build_resume.py`
  is a layout-only renderer, and all content — the bullet pool, per-lane
  titles, summaries, and ordering — lives in a YAML content file
  (`framework/templates/resume_content.yml` is the blank shape). A
  positioning-strategy change is a data edit, not a code change. The
  remaining softness: the renderer still reads its own content file rather
  than deriving anything from `profile.yml` or `experience_summary.md`
  directly, so the honesty link between bullets and the verified summary is
  a discipline, not a mechanical check. The content file also carries a
  two-layer skills ledger — `skills` (a group→item→children tree whose nodes
  can hold depth, years, and the canonical spelling, rendered names-only) and
  an off-resume `context_ledger` — which doubles as the match/gap source of
  truth so vetting resolves a JD's stack against recorded facts, not recall.
