...
| `private/data/rituals.yml` | Last-run stamps for non-daily rituals (weekly review, resume release, and so on), machine-managed by the daily-log tool, which warns at session start when a cadence has elapsed. A ritual may carry `anchor: <weekday>` instead of a rolling cadence; the weekly review uses `anchor: friday`, coming due each Friday, running at that day's `close`, and committed separately as `weekly-review: <date>`. |
